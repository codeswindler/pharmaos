...
type Pharmacy = { id: number; name: string; email?: string | null; phone?: string | null; status: string; planType: string; planValue: string; userCount: number; smsWalletBalance: number; pendingSmsTopUp: number; mpesa?: { enabled: boolean; environment: string; credentialsVerifiedAt?: string | null } | null };

export default function PharmaciesList() {
  const { token } = useAuth();
  const [pharmacies, setPharmacies] = useState<Pharmacy[]>([]);
  const [search, setSearch] = useState("");
  const load = () => fetch("/api/admin/pharmacies", { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()).then(setPharmacies);
  ...
  return <div>
    ...
    <div className="rounded-lg border border-white/10 overflow-hidden"><table className="w-full text-sm text-white"><thead className="bg-white/5 text-white/40"><tr><th className="text-left p-4">Pharmacy</th><th className="text-left p-4">Staff</th><th className="text-left p-4">M-PESA</th><th className="text-left p-4">SMS wallet</th><th className="text-left p-4">Status</th><th className="p-4" /></tr></thead><tbody>{filtered.map(p => <tr key={p.id} className="border-t border-white/5"><td className="p-4"><div className="flex gap-3 items-center"><Building2 size={16} className="text-green-400" /><div><b>{p.name}</b><p className="text-xs text-white/40">{p.email || p.phone || "No contact"}</p></div></div></td><td className="p-4"><span className="flex gap-1 items-center"><Users size={13} />{p.userCount}</span></td><td className="p-4"><span className="flex gap-1 items-center"><ShieldCheck size={13} className={p.mpesa?.enabled ? "text-green-400" : "text-white/30"} />{p.mpesa?.enabled ? `${p.mpesa.environment} enabled` : "Not enabled"}</span></td><td className="p-4">KES {Number(p.smsWalletBalance || 0).toLocaleString()}{p.pendingSmsTopUp > 0 && <p className="text-xs text-amber-300">KES {p.pendingSmsTopUp.toLocaleString()} requested</p>}</td><td className="p-4 capitalize">{p.status}</td><td className="p-4"><div className="flex justify-end gap-2"><Link href={`/admin/pharmacies/${p.id}/messages`}><button title="SMS audit" className="p-2 border border-white/10 rounded"><MessageSquare size={13} /></button></Link><Link href={`/admin/pharmacies/${p.id}/edit`}><button className="p-2 border border-white/10 rounded"><Pencil size={13} /></button></Link>{p.status !== "suspended" && <button onClick={() => suspend(p.id)} className="p-2 border border-white/10 rounded text-red-400"><XCircle size={13} /></button>}</div></td></tr>)}</tbody></table></div>
  </div>;
}
